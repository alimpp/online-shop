  ...
  <div class="dataHats">
     ...
     <div class="row app-flex-justify-content opa-anim" v-if="!loading">
        ...
        <div class="app-cart my-2 mx-2 pointer opa-anim" v-for="data in dataSource" :key="data.id">
           <router-link :to="`/hats/${data.id}`" class="app-flex-justify-content link">
                ...
                <div class="app-cart-text px-3 app-flex-justify-content">
                  <router-link :to="`/hats/${data.id}`" class="app-flex-column link">
...
export default {
   ...
   computed : {
    dataSource(){
        const products =  this.$store.getters['HatsStore/getData']
