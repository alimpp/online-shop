    ...
    <div class="container" style="height: 95vh;">
      <div class="row mt-5">
        ...
        <div class="col-lg-6 app-flex-column app-flex-justify-content px-5 mt-2">
          ...
          <span class="app-text-size-z app-light-color">{{singleHats.name}}</span>
          <span class="app-text-size-x app-light-color">Size</span>
          <span class="app-text-size-y app-light-color">{{singleHats.size}}</span>
          <span class="app-text-size-x app-light-color">Price</span>
          <span class="app-text-size-y app-light-color">{{singleHats.price}} $</span>
          ...
          <div>
            <button class="app-btn-blue mt-2" @click="addToCart(singleHats)">Add To Cart</button>
            <button class="app-btn-yellow mt-2 mx-2" @click="back">Back To Hats</button>
          </div>
        </div>
      </div>
...
export default {
    ...
    mounted(){
      this.hats = this.$store.getters['HatsStore/getData']
      this.singleHats = this.hats.find(h => h.id == this.$route.params.id)
    } , 
    methods : {
      descriptions(){
