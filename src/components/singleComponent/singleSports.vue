<template>
  <div class="singleSports opa-anim">
    <div class="container" style="height: 95vh;">
      <div class="row mt-5">
        <div class="col-lg-6 app-flex-justify-content">
          <img :src="singleSports.image" alt="Error" style="width:70%; height:330px; border-radius:5px;">
        </div>
        <div class="col-lg-6 app-flex-column app-flex-justify-content px-5 mt-2">
          ...
          <span class="app-text-size-z app-light-color">{{singleSports.name}}</span>
          <span class="app-text-size-x app-light-color">Size</span>
          <span class="app-text-size-y app-light-color">{{singleSports.size}}</span>
          <span class="app-text-size-x app-light-color">Price</span>
          <span class="app-text-size-y app-light-color">{{singleSports.price}} $</span>
          ...
          <div>
            <button class="app-btn-blue mt-2" @click="addToCart(singleSports)">Add To Cart</button>
...
export default {
    ...
    mounted(){
      this.sports = this.$store.getters['SportsStore/getData']
      this.singleSports = this.sports.find(s => s.id == this.$route.params.id)
    } , 
    methods : {
      descriptions(){
      ...
      back(){
        return this.$router.push('/sports')
      }
    }
}
</script>
